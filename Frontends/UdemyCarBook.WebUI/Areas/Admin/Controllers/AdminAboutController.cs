...
namespace UdemyCarBook.WebUI.Areas.Admin.Controllers
{
        ...
        public class AdminAboutController : Controller
        {
            ...
            public async Task<IActionResult> CreateAbout(CreateAboutDto createAboutDto)
            {
                ...
                var responseMessage = await client.PostAsync("https://localhost:7026/api/Abouts", stringContent);
                if (responseMessage.IsSuccessStatusCode)
                {
            ...
            {
                var client = _httpClientFactory.CreateClient();
                var resposenMessage = await client.GetAsync($"https://localhost:7026/api/Abouts/{id}");
            ...
            public async Task<IActionResult> UpdateAbout(UpdateAboutDto updateAboutDto)
            {
                ...
                StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
                var responseMessage = await client.PutAsync("https://localhost:7026/api/Abouts/", stringContent);
                if (responseMessage.IsSuccessStatusCode)
                {
                    return RedirectToAction("Index", "AdminAbout", new { area = "Admin" });
                }
